...
windows_firewall::exception { 'Choco server':
  ...
  description  => 'Inbound rule for Chocolatey Servert. [TCP 8080]',
}
...
exec { 'VirtualDir':
  command   => '$(Import-Module WebAdministration;New-Item IIS:\Sites\chocolatey.server\App_Data\Packages -type VirtualDirectory -physicalPath \\\\burn.itg.ias.edu\\packages$ -Force)',
  provider  => powershell,
  logoutput => true,
  require => Class["chocolatey_server"],
}
